...
read_charxl_data("0920_.xlsx") %>% nrow
...
# csv_convertAllText2Numeric_oversized <- function (file_)
# {
#     read_charxl_full(paste0(file_,".xlsx")) %>%
#         select(1:11) %>% 
#         mutate(AmountNumeric = suppressWarnings(as.numeric(.[[ncol(.)]]))) %>% # last col to int
#         mutate(AmountRoundup = round(.[[ncol(.)]])) %>% # round last col
#         select(1:11, ncol(.)) %>% # select 10 + Corrected Amount
#         filter(suppressWarnings(!is.na(AmountRoundup))) %>% 
#         mutate_at(.cols = 1:10, .funs = funs(as.integer_spprssWrns(.))) %>%
#         select(1:11) %>% 
#         write.csv(paste0(file_,"_oversized.csv"), na = "", row.names = F)
# }
# 
